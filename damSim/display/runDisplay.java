...
public class runDisplay {
	...
	private static SnowyScheme constructSnowyScheme() {
		...
		// Rivers
		River hightoBlowering = new River("Blowering to Ocean", 10000, 100 , 500, 10, scheme.getOcean()); // From blowering to ocean
		blowering.connectTo(hightoBlowering);
		scheme.addRiver(hightoBlowering);
		River junamaToblowering = new River("Junama To Blowering", 10000, 100 , 500, (float) 17.857, blowering);
		jounama.connectTo(junamaToblowering);
		scheme.addRiver(junamaToblowering);
		River talbingoTojunama = new River("Talbingo to Junama", 10000, 100 , 500, (float) 8.923, jounama);
		talbingo.connectTo(talbingoTojunama);
		scheme.addRiver(talbingoTojunama);
		River tumutToTalbingo = new River("Tumut to Talbingo", 10000, 100 , 500, (float) 29.762, talbingo);
		tumut2.connectTo(tumutToTalbingo);
		scheme.addRiver(tumutToTalbingo);
		River tumutToTamut = new River("Tumut 1 to Tumut 2", 10000, 100 , 500, (float) 8.929, tumut2);
		tumut1.connectTo(tumutToTamut);
		scheme.addRiver(tumutToTamut);
		River toomaToTumut = new River("Tooma to Tumut 1", 10000, 100 , 500, (float) 14.881, tumut1);
		tooma.connectTo(toomaToTumut);
		scheme.addRiver(toomaToTumut);
		River happyJToTumut = new River("Happy Jacks to Tumut 1", 1000, 0, 50, (float) 14.881, tumut1);
		...
		River tangaraToEucumbene = new River("Tangara to Eucumbene", 10000, 0 , 50, (float) 35.714, eucumbene);
		tangara.connectTo(tangaraToEucumbene);
		scheme.addRiver(tangaraToEucumbene);
		River eucumbeneToJindabyne = new River("Eucumbene To Jinabyne", 10000, 0 , 50, (float) 327.382, jindabyne);
		eucumbene.connectTo(eucumbeneToJindabyne);
		scheme.addRiver(eucumbeneToJindabyne);
		River snowyRiver = new River("Snowy River", 10000, 0 , 50, 100, scheme.getOcean());
		...
		River guthegaToIsland = new River("Guthega to Island Bend", 10000, 0 , 50, (float) 17.857, islandBendDam);
		guthega.connectTo(guthegaToIsland);
		scheme.addRiver(guthegaToIsland);
		River islandToGeehi = new River("Island Bend to Geehi", 10000, 0 , 50, (float) 16.071, geehi);
		islandBendDam.connectTo(islandToGeehi);
		scheme.addRiver(islandToGeehi);
		River geehiToMurray = new River("Geehi to Murray", 10000, 0 , 50, (float) 14.881, murray);
		geehi.connectTo(geehiToMurray);
		scheme.addRiver(geehiToMurray);
		River murrayToKhancoban = new River("Murray to Khancoban", 100000, 100 , 500, (float) 10.119, khancoban);
		murray.connectTo(murrayToKhancoban);
		scheme.addRiver(murrayToKhancoban);
		River khancobanRiver = new River("Khancoban to Ocean", 10000, 0 , 50, 100, scheme.getOcean());
